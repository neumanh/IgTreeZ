...
def get_od_array(t: Tree):
    ...
    # Initiate variables
    od_array = []
    sod_array = []  # Split node out degree
    dasn = []
    drsn = []
    nodes_num = 0
    ...
    for node in t.traverse():
        ...
        if node not in leaves:
            ...
            od_array.append(num_children)  # The number of sons
            if num_children > 1:  # A split node
                sod_array.append(num_children)  # The number of sons
        ...
        if not node.is_root():
            ...
            if (len(node.get_children()) > 1) and (len(node.up.get_children()) > 1):
    ...
    return nodes_num, np.array(od_array), np.array(sod_array), np.array(dasn), np.array(drsn)
...
def get_pl_arrays(t: Tree, first_split_node: Tree):
    ...
    for leaf in leaves:
        ...
        if not first_split_node.is_leaf():  # The first split node is not a leaf
...
def get_one_tree_attr(t: Tree):
    ...
    if t and len(list(t)) > 0:  # If there is more than one node
        ...
        nodes, od_array, sod_array, dasn_array, drsn_array = get_od_array(t)  # Half of the time
